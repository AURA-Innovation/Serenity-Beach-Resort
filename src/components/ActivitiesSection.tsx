      ...
      >
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 mb-6">
          {FILTERS.map((f) => (
            <Button
              ...
              className={`filter-tab ${filter === f ? "active bg-[#007bff] hover:bg-[#0056b3]" : ""} btn-lux btn-gold-hover relative`}
              onClick={() => setFilter(f)}
              size="sm"
            >
              {f}
        ...
        <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {filtered.map((act, i) => {
            ...
            <article
              key={act.id}
              className={`group rounded-xl border parallax-glass shadow-sm flex flex-col overflow-hidden scroll-fade ${inView ? "" : "opacity-0"}`}
              style={{ 
                animation: inView ? `fadeUp 700ms ease-out ${i * 70}ms forwards` : "none",
                opacity: cardOpacity,
                backdropFilter: `blur(${16 + cardBlur}px)`,
